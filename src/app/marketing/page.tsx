...
export default function MarketingPage() {
  const marketingData = mockTermsContent.find(terms => terms.id === 'marketing');

  if (!marketingData) {
    return <div>마케팅 정보 수신 동의 내용을 찾을 수 없습니다.</div>;
  }
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900">{marketingData.title}</h1>
          <p className="text-gray-600 mt-2">
            최종 업데이트: {new Date(marketingData.lastUpdated).toLocaleDateString('ko-KR')}
          </p>
        </div>

        {/* 내용 */}
        <div className="prose prose-lg max-w-none">
          {marketingData.content.split('\n').map((line, index) => {
            if (line.startsWith('# ')) {
              return (
