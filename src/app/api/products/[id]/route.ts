...
export async function GET(
  ...
  try {
    ...
    const sellersDto: SellerInfoResponseDto[] = (rpcResult.sellers || []).map((seller: { seller_name?: string; price: number; original_price?: number; shipping_fee?: number; shipping_fee_threshold?: number; store_url: string }) => ({
      name: seller.seller_name || 'Unknown Seller',
      price: seller.price,
      originalPrice: seller.original_price,
      shippingFee: seller.shipping_fee || 0,
      shippingFeeThreshold: seller.shipping_fee_threshold,
