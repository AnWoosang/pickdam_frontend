...
import {
  ...
  MyPostResponseDto
} from '@/domains/user/types/dto/mypage/mypageDto';
import {
  toUserStats,
  toMyPost,
  toMyComment,
  toMyReview
} from '@/domains/user/types/dto/mypage/mypageMapper';

export const mypageApi = {
  // 사용자 리뷰 조회 (페이지네이션 지원)
  async getUserReviews(userId: string, page: number, limit: number): Promise<PaginationResult<Review>> {
    const response = await apiClient.get<PaginatedResponse<MyReviewResponseDto>>(`${API_ROUTES.USERS.REVIEWS(userId)}?page=${page}&limit=${limit}`);
    return {
  ...
  async getUserStats(userId: string): Promise<UserStats> {
    const response = await apiClient.get<MypageStatsResponseDto>(API_ROUTES.USERS.STATS(userId));
